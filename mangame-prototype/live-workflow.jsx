...
function LiveWorkflow({onHome,onExample}) {
  ...
  async function generate(text,retry){
    ...
    const body={session:sid.current,request_id:crypto.randomUUID(),prompt:text+'\n\n'+AUTO_STYLE_NOTE,quality,use_style:style,confirm_cost:true};
    request.current=body;setJob(null);setError('');setOperation('submitting');
    if(retry)generation();setOperation('submitting');
    try{await accept(await api('/api/image-generate',body,20000));}
  ...
  async function content(d=job){
    try{const data=await api('/api/image-content',{session:sid.current,request_id:d.request_id},30000);if(!alive.current)return;const bytes=Uint8Array.from(atob(data.image_base64),c=>c.charCodeAt(0));if(url.current)URL.revokeObjectURL(url.current);url.current=URL.createObjectURL(new Blob([bytes],{type:'image/png'}));setImageUrl(url.current);setStage('result');setError('');window.scrollTo(0,0);}
